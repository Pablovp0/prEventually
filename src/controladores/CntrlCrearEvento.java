...
public class CntrlCrearEvento implements ActionListener {
	...
	public void popUpError(JPanel parent, String err) {
		JOptionPane.showMessageDialog(parent, err, "Mensaje", JOptionPane.ERROR_MESSAGE);
	}
	...
	public void actionPerformed(ActionEvent e) {
		Evento ev = new Evento(0, cePanel.getNombre().getText(), cePanel.getFecha().getText(),
				cePanel.getLugar().getText(), CntrlInicioSesion.getSesion().getNusuario());

		InterfazListaEventos iLePanel = ip.getPanelListaEventos();

		if (ev.getFecha() == null || ev.getFecha().length() == 0 || ev.getLugar() == null || ev.getLugar().length() == 0
